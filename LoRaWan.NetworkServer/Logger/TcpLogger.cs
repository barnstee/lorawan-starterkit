...
namespace LoRaWan.NetworkServer.Logger
{
    ...
    public static class TcpLoggerExtensions
    {
        ...
        private sealed class TcpLogger(LogLevel logLevel, IExternalScopeProvider externalScopeProvider, Action<string> logger) : ILogger
        {
            ...
            {
                return externalScopeProvider is { } scopeProvider ? scopeProvider.Push(state) : NoopDisposable.Instance;
            }
...
#pragma warning restore format
            ...
            private void Log(string message)
            {
                ...
                ObjectDisposedException.ThrowIf(cancellationTokenSource.Token.IsCancellationRequested, nameof(TcpLoggerProvider));

                _ = this.channel.Writer.TryWrite(formatter?.Invoke(message) ?? message);
            }
                        ...
                        foreach (var line in lines)
                        {
                            for (var attempt = 1; ; attempt++)
                            {
                                try
                                {
                                    ...
                                    var size = encoding.GetByteCount(line ?? string.Empty) + 2;
