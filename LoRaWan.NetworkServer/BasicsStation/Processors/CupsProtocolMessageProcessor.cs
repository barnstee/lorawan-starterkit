...
#pragma warning restore IDE0010 // Add missing cases
                ...
                try
                {
                    ...
                    updateRequest = JsonConvert.DeserializeObject<CupsUpdateInfoRequest>(json);
                }
                ...
                var remoteCupsConfig = await basicsStationConfigurationService.GetCupsConfigAsync(updateRequest?.StationEui, token);

                var firmwareLength = 0U;

                if (!string.Equals(updateRequest?.Package, remoteCupsConfig.Package, StringComparison.OrdinalIgnoreCase))
                {
                    if (updateRequest?.KeyChecksums.Any(c => c == remoteCupsConfig.FwKeyChecksum) == false)
                    ...
                    (firmware, firmwareLength) =
                        await deviceAPIServiceBase.FetchStationFirmwareAsync(updateRequest?.StationEui, token) switch
                        {
                ...
                async Task<byte[]> FetchCredentialsAsync(ConcentratorCredentialType type) =>
                    Convert.FromBase64String(await deviceAPIServiceBase.FetchStationCredentialsAsync(updateRequest?.StationEui, type, token));
