...
namespace LoRaWan.NetworkServer
{
        ...
        Meter meter) : IClassCDeviceMessageSender
    {
        ...
        public async Task<bool> SendAsync(IReceivedLoRaCloudToDeviceMessage message, CancellationToken cts = default)
        {
            ...
            var loRaDevice = await loRaDeviceRegistry.GetDeviceByDevEUIAsync(devEui);
            if (loRaDevice == null)
            {
            ...
            else
            {
                try
                {
                    await downstreamMessageSender.SendDownstreamAsync(downlinkMessageBuilderResp.DownlinkMessage);
                }
                catch (Exception ex)
                {
