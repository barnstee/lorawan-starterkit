...
#pragma warning restore CA5401 // Do not use CreateEncryptor with non-default IV
            ...
            Mic = MessageIntegrityCode.Read(inputMessage.AsSpan(inputMessage.Length - 4, 4));
        }

        public byte[] Serialize(AppKey appKey)
        {
            MessageIntegrityCode mic;
            Mic = mic = MessageIntegrityCode.ComputeForJoinAccept(appKey, MHdr, AppNonce, NetId, DevAddr, DlSettings, RxDelay, CfList);
            ...
            pt = NetId.Write(pt);
            pt = DevAddr.Write(pt);
            DlSettings.Span.CopyTo(pt);
            pt = pt[DlSettings.Span.Length..];
            pt = RxDelay.Write(pt);
            channelFrequencies.CopyTo(pt);
            pt = pt[channelFrequencies.Length..];
            _ = mic.Write(pt);
